/**
 * sticker-illustrations — React Native port of the dashboard's
 * `StickerIllustrations.jsx`. Same six characters (DinoBlue, DinoPink,
 * Stegosaurus, Daisy, Tulip, Sunflower) drawn as inline `react-native-
 * svg` components so the parent / staff app's auth screens can reuse
 * the dashboard's "nursery flashcard" decoration set.
 *
 * Visual language follows the owner's reference (pastel-blue
 * brontosaurus on a soft mint card with white polka dots, friendly
 * closed-eye smile). Hallmarks:
 ...
 *
 * The drawings here are 1:1 ports of the web SVGs — same paths, same
 * fills, same strokes — so the parent / staff app and the owner
 * dashboard render the same characters at the same proportions. If you
 * tweak a sticker, change BOTH files in lockstep.
 *
 * Each component takes a `width` prop (defaults match the dashboard's
 * defaults) and computes height from the SVG's intrinsic aspect ratio.
 */
...
// ============================================================
// DINOSAURS — three pastel variants for variety. All use the same
// silhouette grammar (rounded body + curled tail + closed-eye smile)
// so the set reads as a cohesive nursery-sticker collection rather
// than mismatched characters.
// ============================================================

/**
 * DinoBlue — pastel-blue brontosaurus, matches the owner's reference
 * almost 1:1 (blue body, white polka dots, gentle closed-eye smile).
 * The "anchor" character of the set.
 */
export function DinoBlue({ width = 120 }: StickerProps) {
  return (
    <Svg width={width} height={(width * 120) / 160} viewBox="0 0 160 120">
      <Path
 ...
 * to balance the blue lead character on the opposite side of the page.
 */
export function DinoPink({ width = 100 }: StickerProps) {
  return (
    <Svg width={width} height={(width * 120) / 160} viewBox="0 0 160 120">
      <Path
        d="M22 78 Q6 70 10 56 Q18 50 28 60 Q34 70 42 76 Z"
 ...
 */
export function Stegosaurus({ width = 110 }: StickerProps) {
  return (
...
// ============================================================
// FLOWERS — three sticker-style flowers (daisy, tulip, sunflower).
// All three include a green stem + leaves so they read as part of the
// same garden, even though the petal shapes differ.
// ============================================================

/**
 * Daisy — six round white petals + yellow center + green stem.
 * Reads as the most "neutral, friendly" flower of the set.
 */
export function Daisy({ width = 56 }: StickerProps) {
  return (
    <Svg width={width} height={(width * 110) / 80} viewBox="0 0 80 110">
      <Path d="M40 42 Q37 65 40 100" stroke="#74C69D" strokeWidth={3} fill="none" strokeLinecap="round" />
      <Path
...
/**
 * Tulip — pink bell shape on a green stem with two leaves. Adds a
 * different silhouette (vertical, narrow) to balance the rounder daisy
 * and sunflower in the scene.
 */
export function Tulip({ width = 48 }: StickerProps) {
  return (
    <Svg width={width} height={(width * 110) / 60} viewBox="0 0 60 110">
      <Path d="M30 50 Q28 75 30 105" stroke="#74C69D" strokeWidth={3} fill="none" strokeLinecap="round" />
      <Path
      ...
      <Path
        ...
        strokeLinejoin="round"
      />
      <Path d="M30 20 Q24 28 24 42 Q28 50 30 50 Q32 50 36 42 Q36 28 30 20 Z" fill="#FFC2DF" />
    </Svg>
  );
}
 ...
 * The "warm anchor" of the flower set; pairs visually with DinoBlue
 * across the page (warm vs cool).
 */
export function Sunflower({ width = 68 }: StickerProps) {
  return (
    <Svg width={width} height={(width * 120) / 90} viewBox="0 0 90 120">
      <Path d="M45 50 Q43 75 45 115" stroke="#74C69D" strokeWidth={3} fill="none" strokeLinecap="round" />
      <Path
